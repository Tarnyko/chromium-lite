...
class SecureHashSHA256NSS : public SecureHash {
 public:
  ...
  std::unique_ptr<SecureHash> Clone() const override { return std::unique_ptr<SecureHash> (new SecureHashSHA256NSS(*this)); }
...
std::unique_ptr<SecureHash> SecureHash::Create(Algorithm algorithm) {
  switch (algorithm) {
    case SHA256:
      return std::unique_ptr<SecureHash> (new SecureHashSHA256NSS());
    default:
      NOTIMPLEMENTED();
      return nullptr;
  }
