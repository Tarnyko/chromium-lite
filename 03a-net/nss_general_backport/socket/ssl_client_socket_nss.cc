...
void AddLogEvent(const base::WeakPtr<NetLogWithSource>& net_log,
...
// SSLClientSocketNSS::Core.
void AddLogEventWithCallback(const base::WeakPtr<NetLogWithSource>& net_log,
                             NetLogEventType event_type,
                             const NetLogParametersCallback& callback) {
  if (!net_log)
    return;
  net_log->AddEvent(event_type, callback);
}

// Helper function to make it easier to call NetLogWithSource::AddByteTransferEvent
...
void LogByteTransferEvent(
    const base::WeakPtr<NetLogWithSource>& net_log, NetLogEventType event_type,
...
class SSLClientSocketNSS::Core : public base::RefCountedThreadSafe<Core> {
 ...
 private:
  ...
  base::WeakPtrFactory< NetLogWithSource> weak_net_log_factory_;
  ...
  base::WeakPtr<NetLogWithSource> weak_net_log_;
...
SECStatus SSLClientSocketNSS::Core::ClientAuthHandler(
  ...
  core->PostOrRunCallback(
      ...
      base::Bind(&AddLogEvent, core->weak_net_log_,
                 NetLogEventType::SSL_CLIENT_CERT_REQUESTED));
...
int SSLClientSocketNSS::Core::DoGetDBCertComplete(int result) {
  ...
  PostOrRunCallback(
      FROM_HERE,
      base::Bind(&NetLogWithSource::EndEventWithNetErrorCode, weak_net_log_,
                 NetLogEventType::SSL_GET_DOMAIN_BOUND_CERT, result));
    ...
    if (rv == 0) {
      PostOrRunCallback(FROM_HERE,
                        base::Bind(&LogByteTransferEvent, weak_net_log_,
                                   NetLogEventType::SSL_SOCKET_BYTES_RECEIVED, rv,
...
int SSLClientSocketNSS::Core::DoPayloadWrite() {
  ...
  if (rv >= 0) {
    PostOrRunCallback(FROM_HERE,
                      base::Bind(&LogByteTransferEvent, weak_net_log_,
                                 NetLogEventType::SSL_SOCKET_BYTES_SENT, rv,
...
SECStatus SSLClientSocketNSS::Core::ClientChannelIDHandler(
  ...
  core->PostOrRunCallback(
      FROM_HERE,
      base::Bind(&AddLogEvent, core->weak_net_log_,
                 NetLogEventType::SSL_CHANNEL_ID_REQUESTED));
  ...
  core->PostOrRunCallback(
      FROM_HERE,
      base::Bind(&NetLogWithSource::EndEventWithNetErrorCode, core->weak_net_log_,
                 NetLogEventType::SSL_GET_DOMAIN_BOUND_CERT, error));
  SECStatus rv = SECSuccess;
  if (error == OK) {
    // Synchronous success.
...
void SSLClientSocketNSS::Core::UpdateServerCert() {
  ...
  if (nss_handshake_state_.server_cert.get()) {
    ...
    PostOrRunCallback(
        FROM_HERE,
        base::Bind(&AddLogEventWithCallback, weak_net_log_,
                   NetLogEventType::SSL_CERTIFICATES_RECEIVED,
                   net_log_callback));
  }
}
...
int SSLClientSocketNSS::Core::DoGetChannelID(const std::string& host) {
  ...
  weak_net_log_->BeginEvent(NetLogEventType::SSL_GET_DOMAIN_BOUND_CERT);
...
void SSLClientSocketNSS::Core::SetChannelIDProvided() {
  PostOrRunCallback(
      FROM_HERE, base::Bind(&AddLogEvent, weak_net_log_,
                            NetLogEventType::SSL_CHANNEL_ID_PROVIDED));
...
// TARNYKO: implemented this
bool SSLClientSocketNSS::WasAlpnNegotiated() const {
#if 0
...
NextProto SSLClientSocketNSS::GetNegotiatedProtocol() const {
#if 0
...
const NetLogWithSource& SSLClientSocketNSS::NetLog() const {
  return net_log_;
}
...
int SSLClientSocketNSS::DoHandshakeComplete(int result) {
  ...
  if (result == OK) {
    ...
    // TARNYKO implemented this
    struct SSLResumptionTokenInfo alpn_info;
    const uint8_t* alpn_proto = NULL;
    unsigned alpn_len = 0;
    // WIP
    SSL_GetResumptionTokenInfo(ssl_.get(), XXXX, &alpn_proto, &alpn_info, &alpn_len);
    if (alpn_len > 0) {
...
int SSLClientSocketNSS::DoVerifyCert(int result) {
  ...
  if (ssl_config_.IsAllowedBadCert(core_->state().server_cert.get(), &cert_status)) {
  ...
  return cert_verifier_->Verify(
     CertVerifier::RequestParams(core_->state().server_cert.get(), host_and_port_.host(),
                                  ssl_config_.GetCertVerifyFlags(),
                                  core_->state().stapled_ocsp_response, CertificateList()),
...
int SSLClientSocketNSS::DoVerifyCertComplete(int result) {
  ...
  if (transport_security_state_ &&
    ...
    if (pin_validity == TransportSecurityState::PKPStatus::VIOLATED)
...
void SSLClientSocketNSS::VerifyCT() {
  ...
  cert_transparency_verifier_->Verify(
      ...
      core_->state().sct_list_from_tls_extension, &ct_verify_result_.scts, net_log_);
  ...
  // TARNYKO implemented this
   SCTList verified_scts = ct::SCTsMatchingStatus(
      ct_verify_result_.scts, ct::SCT_STATUS_OK);
  //
    ...
    if ((server_cert_verify_result_.cert_status & CERT_STATUS_IS_EV)) {
      ...
      ct::EVPolicyCompliance ev_policy_compliance =
          policy_enforcer_->DoesConformToCTEVPolicy(
              server_cert_verify_result_.verified_cert.get(),
              ev_whitelist.get(), verified_scts, net_log_);
...
Error SSLClientSocketNSS::GetTokenBindingSignature(
...
bool SSLClientSocketNSS::Core::IsChannelIDEnabled() const {
