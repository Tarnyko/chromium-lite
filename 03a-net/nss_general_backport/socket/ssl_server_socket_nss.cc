...
const NetLogWithSource& SSLServerSocketNSS::NetLog() const {
  return net_log_;
}
...
bool SSLServerSocketNSS::WasAlpnNegotiated() const {
  NOTIMPLEMENTED();
  return false;
}
...
int SSLServerSocketNSS::DoPayloadRead() {
  ...
  net_log_.AddEvent(NetLogEventType::SSL_READ_ERROR,
...
int SSLServerSocketNSS::DoPayloadWrite() {
  ...
  rv = MapNSSError(prerr);
  net_log_.AddEvent(NetLogEventType::SSL_WRITE_ERROR,
                    CreateNetLogSSLErrorCallback(rv, prerr));
  return rv;
}
  ...
  if (rv == SECSuccess) {
    ...
    if (net_error == ERR_IO_PENDING) {
      ...
      LOG(ERROR) << "handshake failed; NSS error code " << prerr
                 << ", net_error " << net_error;
      net_log_.AddEvent(NetLogEventType::SSL_HANDSHAKE_ERROR,
